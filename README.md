...
A Chrome extension that turns articles into a clean, focused reading view, inspired by the reader experience built into Microsoft Edge. It uses Mozilla Readability to identify the main text generically, so it does not depend on publisher-specific CSS selectors. It does not transmit article text to an external reading or analytics service and does not collect browsing data.
...
Read aloud uses Chrome's built-in Web Speech support and the voices configured on the computer. The article is divided into short passages for reliable playback; no external speech or analytics service is added by Local Reader.
